...
from packages.open_aea.protocols.signing.custom_types import RawTransaction
...
from packages.valory.skills.transaction_settlement_abci.payload_tools import (
    ...
    tx_hist_payload_to_hex, RelayedTxStatus,
)
...
class TransactionSettlementBaseBehaviour(BaseBehaviour, ABC):
    ...
    def send_raw_transaction_relayer(
        self,
        ...
        **kwargs: Any,
        ...
        """Send a raw transaction via the relayer."""
        body = json.dumps(transaction.body, sort_keys=True).encode("utf-8")
        submit_endpoint = f"{self.params.relayer_endpoint}/api/relay/submit"
    ...
    def _get_tx_data(
        ...
        send_raw_transaction = (
            self.send_raw_transaction_relayer
...
class ValidateTransactionBehaviour(TransactionSettlementBaseBehaviour):
    ...
    def _get_tx_hash_to_validate(self) -> Generator[None, None, Optional[str]]:
        """Get the transaction hash to validate."""
        if not self.params.use_relayer:
        ...
        while True:
            ...
            if response.status_code != 200:
                self.context.logger.error(
                    f"Could not retrieve the transaction hash from the relayer. "
                    f"Received status code {response.status_code}."
                )
                return None
