...
class GnosisSafeContract(Contract):
    ...
    def _verify_relayed_tx(
        ...
        """
        Verify a relayed transaction.

        :param ledger_api: the ledger API object
        :param contract_address: the contract address
        :param tx_hash: the transaction hash
        :param safe_tx_hash: the Safe transaction hash
        :return: the verified status
        """
        safe_contract = cls.get_instance(ledger_api, contract_address)
        try:
            ...
            safe_tx = safe_contract.events.ExecutionSuccess().process_receipt(receipt)[0]
            verified = (
                safe_tx["args"]["txHash"] == safe_tx_hash and
                ledger_api.api.to_checksum_address(contract_address)
                == ledger_api.api.to_checksum_address(safe_tx["address"])
            )
            return dict(
                verified=verified,
            ...
            _logger.error(f"Error processing receipt: {e}")
            failed_tx = 0
            return dict(
                verified=False,
                status=failed_tx,
    ...
    def verify_tx(  # pylint: disable=too-many-arguments,too-many-locals
        ...
        refund_receiver: str = NULL_ADDRESS,
        safe_version: Optional[str] = None,
        is_via_relayer: bool = False,
        safe_tx_hash: Optional[bytes] = None,
    ) -> JSONLike:
        """
        ...
        :param refund_receiver: Address of receiver of gas payment (or `0x000..000`  if tx.origin).
        :param safe_version: Safe version 1.0.0 renamed `baseGas` to `dataGas`. Safe version 1.3.0 added `chainId` to the `domainSeparator`. If not provided, it will be retrieved from network
        :param is_via_relayer: whether the transaction is via a relay
        :param safe_tx_hash: the Safe transaction hash
        :return: the verified status
        """
        if is_via_relayer:
            return cls._verify_relayed_tx(
                ledger_api, contract_address, tx_hash, safe_tx_hash
            )
